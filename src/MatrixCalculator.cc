 ...
 Matrix *MatrixCalculator::cumSum(Matrix* ts)
{
 ...
 Matrix *MatrixCalculator::cumProd(Matrix* ts)
{
    int len = ts->getNCol();
    Matrix *res = new Matrix(1, len);
    double cumprod = 0;
 ...
 double MatrixCalculator::summaryGini(Matrix* ts, int ngroup)
{
    ...
    //sort the sum array
    std::sort(sumArray, sumArray + len);
    
    double height = 0;
    double area = 0;
    for(int i = 0; i < ngroup; i++){
        double val = sumArray[i];
        height += val;
        area += height;
    }

    double gini = 1.0 - area / (height * ngroup / 2.0);
